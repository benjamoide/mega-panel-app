...
class Tratamiento:
    def __init__(self, id_t, nombre, zona, ondas, intensidad, distancia, duracion, max_diario, tiempo_espera_horas, tipo, tags_entreno, momento_categoria, momento_ideal_txt, fases_info=None):
        self.id = id_t
        self.nombre = nombre
        self.zona = zona
        self.ondas = ondas
        ...
        self.momento_categoria = momento_categoria # Para agrupar visualmente (PRE, POST, FLEX, NIGHT...)
...
def obtener_catalogo():
    ...
    catalogo = [
        ...
        # --- LESIONES (FLEXIBLE) ---
        Tratamiento("rodilla_d", "Rodilla Derecha (Lesión)", "Rodilla Dcha", "NIR + RED", "100%", "15-20 cm", 10, 2, 6, "LESION", ['All'], "FLEX", "Flexible: Antes (Calor) o Después (Alivio)", fases_articulacion)
        .set_incompatibilidades("Implantes metálicos, Cáncer activo."),
        
        Tratamiento("rodilla_i", "Rodilla Izquierda (Lesión)", "Rodilla Izq", "NIR + RED", "100%", "15-20 cm", 10, 2, 6, "LESION", ['All'], "FLEX", "Flexible: Antes (Calor) o Después (Alivio)", fases_articulacion)
        .set_incompatibilidades("Implantes metálicos, Cáncer activo."),
        
        Tratamiento("codo_d", "Codo Derecho (Lesión)", "Codo Dcho", "NIR + RED", "100%", "15-20 cm", 10, 2, 6, "LESION", ['All'], "FLEX", "Flexible: Antes (Calor) o Después (Alivio)", fases_articulacion)
        .set_incompatibilidades("No usar si infiltración <5 días."),
        
        Tratamiento("codo_i", "Codo Izquierdo (Lesión)", "Codo Izq", "NIR + RED", "100%", "15-20 cm", 10, 2, 6, "LESION", ['All'], "FLEX", "Flexible: Antes (Calor) o Después (Alivio)", fases_articulacion)
        .set_incompatibilidades("No usar si infiltración <5 días."),
        
        # --- MÚSCULO (POST-ENTRENO) ---
...
grupos_visuales = {
    ...
    "NIGHT": [],
    "ANY": [],
    "HIDDEN": [] # Para los que no aplican hoy
}
...
for t in lista_tratamientos:
    ...
    if mostrar:
        grupos_visuales[t.momento_categoria].append((t, es_ciclo_activo))
    else:
        grupos_visuales["HIDDEN"].append((t, False))

# --- RENDERIZADO POR SECCIONES ---

# Función auxiliar para renderizar una tarjeta de tratamiento
def render_tratamiento(t, es_ciclo_activo):
    # Info Fase Lesión
    info_fase = ""
        ...
        if not completo and not bloqueado_por_fin:
            ...
            if not bloqueado_tiempo:
                ...
                if t.tipo in ["LESION", "MUSCULAR"] and seleccion_rutinas:
                    detalle_sel = st.radio("¿Cuándo?", ["Antes de Entrenar", "Después de Entrenar", "Otro"], horizontal=True, key=f"rad_{t.id}")
                elif t.tipo == "GRASA":
...
# 2. FLEXIBLES (Lesiones)
if grupos_visuales["FLEX"]:
    st.markdown("### ⚖️ Flexible (Lesiones: Antes o Después)")
    for t, ciclo in grupos_visuales["FLEX"]:
        render_tratamiento(t, ciclo)

# 3. POST-ENTRENO
if grupos_visuales["POST"]:
    st.markdown("### 🧘 Después de Entrenar (Recuperación)")
...
if grupos_visuales["HIDDEN"]:
    st.markdown("---")
    with st.expander("📂 Tratamientos No Prioritarios Hoy (Ver/Activar)"):
